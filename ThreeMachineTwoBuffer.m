function [Profit] = ThreeMachineTwoBuffer(Time,TransMatPower,TransMatIdlePower)
...
%         TransMatPre{1}=TransMatPre{1}*TransMat{1}^t;
        TransMatPre{1}=TransMatPower{1}(4*t-3:4*t,:);
        Temp=rand;
    ...
    for i=2:Max%第一个机器单独一段，在上面已经计算过花费和产品，下面进入下一段计算，计算一个缓冲区加一台机器的花费和产品
        if Num(i-1)~=0%上一段机器正常运转
            if Condition(i)<ConTh(i)%上一段机器正常运转且2机器正常运转
                ...
                %更新机器2的状态
                TransMatPre{i}=TransMatPre{i}*TransMatPower{i}(4*t-3:4*t,:);
                Temp=rand;
                ...
                if IsFull==0
                    TransMatPre{i-1}=TransMatPre{i-1}*TransMatIdlePower{i-1}(4*t-3:4*t,:);
                    Temp=rand;
            ...
            if Condition(i)<ConTh(i)%2机器正常运转
                ...
                if IsEmpty==0
                    ...
                    TransMatPre{i}=TransMatPre{i}*TransMatIdlePower{i}(4*t-3:4*t,:);
                end
                Temp=rand;
